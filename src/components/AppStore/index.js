...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, inputElement: ''}
  ...
  getSearchResult = () => {
    ...
    const searchResult = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(inputElement.toLowerCase()),
    )
    return searchResult
  }

  render() {
    const {inputElement, activeTabId} = this.state
    const searchResult = this.getSearchResult()
    const filterAppList = this.getFilterAppList(searchResult)
